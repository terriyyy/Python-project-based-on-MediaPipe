...
4. **🍎 水果忍者**：挥动手指切开屏幕上的水果，获得分数，注意躲避炸弹。
5. **⚔️ 街头霸王**： 指尖掌控移动，挥拳触发大招！体验硬核体感格斗
6. **🎨 你画我猜**：伸出食指空中作画，让神经网络猜猜你画了什么！
7. **✍️ 指尖绘画**：限定时间，随机图形，获取分数。尝试一下隔空画画吧！
8. **🌟 摘星之手**：用食指摘取飞落的星星，不要让它坠落到地面。
   
## 🚀 安装与运行

### 环境要求
- Python 3.8+(过高的版本会导致运行失败！)
- 摄像头（内置或外接）
